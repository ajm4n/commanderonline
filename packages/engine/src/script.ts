...
export type Amount =
  ...
  /** "where X is the number of players being attacked" — distinct defending players this combat. */
  | { kind: 'playersBeingAttacked' }
...
export type Condition =
  ...
  | { kind: 'mostCommonColor'; color: Color; orTied?: boolean }
  | { kind: 'notYourTurn' }
...
export type Effect =
  ...
  | { kind: 'becomeCopy'; what: Ref; of: Ref; exceptions?: TokenSpec['exceptions']; /** Temporary copy ("until end of turn"); permanent when omitted. */ duration?: Duration }
  ...
  | { kind: 'setColors'; colors: Color[]; on: Ref; duration?: Duration; /** "the color or colors of your choice" */ chooseColors?: boolean; /** Use the colour stored under this memory key instead of `colors`. */ chosenKey?: string; /** "is black in addition to its other colors": keep the colours it already has. */ add?: boolean }
  ...
  | { kind: 'addMana'; mana: ManaColor[] | 'anyColor' | 'anyOneColor' | 'commanderColors' | 'chosenColor' | 'triggerMana' | /** Chrome Mox: one of the colors of the cards exiled with this. */ 'exiledColors'; amount?: Amount; who?: Ref }
  ...
  | { kind: 'searchLibrary'; who?: Ref; filter: ObjectFilter; count: Amount; /** Bribery: the found card enters under this effect's controller rather than the searcher. */ controller?: 'you'; /** `hold`: leave the found cards where they are and remember them under `key` for follow-up sentences. */ destination: 'hand' | 'battlefield' | 'top' | 'graveyard' | 'exile' | 'hold'; key?: string; tapped?: boolean; reveal?: boolean; shuffle?: boolean; /** "search your graveyard, hand, and/or library" */ zones?: ('library' | 'graveyard' | 'hand')[]; /** Pick at random instead of choosing ("return a card at random from your graveyard"). */ random?: boolean }
  ...
  | { kind: 'playFromExile'; what: Ref; /** 'untilYourNextTurn' ends when your next turn begins; 'untilEndOfYourNextTurn' lasts through that turn. */ duration?: 'thisTurn' | 'permanent' | 'untilYourNextTurn' | 'untilEndOfYourNextTurn'; /** Airbend: castable for this cost instead of its mana cost. */ forCost?: string; /** "If you cast a spell this way, pay life equal to its mana value rather than pay its mana cost." */ payLife?: boolean; /** The owner may cast it, not this effect's controller. */ owner?: boolean; /** Granted flashback: castable from the graveyard. */ fromGraveyard?: boolean; /** Exile it as it resolves. */ exileAfter?: boolean; /** "mana of any type can be spent to cast that spell" */ anyMana?: boolean; /** "without paying their mana costs" */ free?: boolean; /** Only cards matching this may be played ("cast noncreature spells from among those cards"). */ filter?: ObjectFilter }
  ...
  | { kind: 'anyPlayerMay'; effects: Effect[]; prompt?: string; /** A cost each player may pay instead of running `effects`. */ cost?: string; /** Runs once if any player paid. */ then?: Effect[]; /** "any opponent may …": the controller is not asked. */ opponentsOnly?: boolean }
  ...
  | { kind: 'revealUntil'; filter: ObjectFilter; /** `hold` leaves the matches where they are and remembers them for follow-up sentences. */ destination: 'hand' | 'battlefield' | 'graveyard' | 'exile' | 'hold'; rest: 'bottom' | 'graveyard' | 'exile' | 'hand' | 'top' | 'shuffle'; tapped?: boolean; who?: Ref; /** Reveal until this many cards match ("until you reveal three nonland cards"). */ count?: Amount; key?: string }
...
export type ReplacementSpec =
  ...
  | { kind: 'replacement'; text: string; event: 'drawCard'; who: 'you' | 'opponent' | 'any'; /** Dredge: the card offering the replacement is in a graveyard. */ zone?: 'graveyard'; /** The player may decline it. */ optional?: boolean; /** Draw this many instead of one. */ draws?: number; /** Replace the draw with these effects entirely. */ effects?: Effect[]; /** "that player skips that draw instead" */ skip?: boolean; condition?: Condition; /** Skip the first draw each of that player's draw steps. */ exceptFirstEachDrawStep?: boolean }
