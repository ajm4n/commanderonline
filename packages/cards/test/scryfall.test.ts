...
describe('fixture', () => {
  it('contains the expected cards and every one is playable', () => {
    expect(raw.length).toBe(84);
    expect(raw.every(isPlayableCard)).toBe(true);
    expect(raw.filter((c) => c.card_faces).map((c) => c.layout).sort()).toEqual(['adventure', 'modal_dfc', 'split', 'transform']);
  });
});
...
describe('CardDb', () => {
  ...
  it('loads the fixture synchronously', () => {
    expect(db.size).toBe(84);
    expect(db.all()).toHaveLength(84);
