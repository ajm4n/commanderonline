...
function parseAdjectives(wordsIn: string[], r: ParsedNoun): boolean {
  ...
  for (const raw of words) {
    ...
    else if (l === 'foretold') r.filter.foretold = true;
...
function applyQualifier(q: string, r: ParsedNoun) {
  ...
  if (q === "from defending player's graveyard" || q === "in defending player's graveyard") {
    ...
    r.isCard = true;
  } else if (/^(?:in|from) (?:your )?opponents'? graveyards$/.test(q)) {
    r.filter.zone = 'graveyard';
  ...
  else if ((m = q.match(/^(?:that (?:was|were) )?dealt damage(?: this turn)? by (?:a|any) source (you|an opponent) control(?:s|led)?(?: this turn)?$/))) r.filter.damagedByFilter = { controller: m[1] === 'you' ? 'you' : 'opponent' };
  else if (/^(?:that (?:was|were) )?dealt damage(?: this turn)? by (?:enchanted|equipped) (?:creature|permanent)(?: this turn)?$/.test(q)) r.filter.damagedByHost = true;
  ...
  else if ((m = q.match(/^except for (.+)$/)) && !/^(?:~|commanders|legendary creatures)$/.test(m[1])) {
    // Qualifiers arrive lowercased, so restore the head word's capital for subtype lookup.
    const inner = parseNoun(`a ${m[1].replace(/^([\w'-]+)/, (w) => w.charAt(0).toUpperCase() + w.slice(1))}`) ?? parseNoun(`a ${m[1]}`);
    if (inner && inner.confident) r.filter.notMatching = { ...inner.filter, zone: undefined };
    else r.confident = false;
  }
  ...
  else if ((m = q.match(/^with mana value (\d+|x)$/))) r.filter.cmcEQ = m[1] === 'x' ? 'X' : parseInt(m[1], 10);
  else if ((m = q.match(/^whose controller was dealt (combat )?damage by (?:~|it) this turn$/))) r.filter.controllerDamagedBySource = m[1] ? 'combat' : 'any';
  else if ((m = q.match(/^that (?:is|are) snow or could produce \{([WUBRGC])\}$/))) r.filter.anyOf = [{ supertypes: ['Snow'] }, { couldProduce: m[1] as import('@commander/engine').ManaColor }]; else if ((m = q.match(/^with a (\+1\/\+1|-1\/-1|loyalty|charge) counter on (?:it|them)$/))) r.filter.hasCounter = m[1];
