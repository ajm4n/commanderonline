...
function applyQualifier(q: string, r: ParsedNoun) {
  ...
  else if ((m = q.match(/^with (awaken|warp|disturb|flashback|madness|embalm|eternalize|bestow|evoke|emerge|escape|plot|rebound|aftermath|retrace|dredge|haunt|miracle|encore|jump-start|adventure|unearth)$/))) r.filter.keywords = [...(r.filter.keywords ?? []), m[1].replace(/^\w/, (c) => c.toUpperCase())];
