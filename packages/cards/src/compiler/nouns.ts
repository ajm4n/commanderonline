...
function applyQualifier(q: string, r: ParsedNoun) {
  ...
  if (q === "from defending player's graveyard" || q === "in defending player's graveyard") {
  ...
  else if (/^that shares a colou?r with (?:~|it)$/.test(q)) r.filter.sharesColorWith = { ref: 'self' };
  ...
  } else if (/^that (?:aren't|are not|isn't|is not) of the chosen type$/.test(q)) r.filter.notChosenSubtypeKey = 'creatureType';
    ...
    r.filter.controller = 'you';
  } else if ((m = q.match(/^with (\w+(?: or \w+)+)$/)) && m[1].split(' or ').every((w) => KEYWORD_QUALS.has(w))) {
    r.filter.keywords = m[1].split(' or ').map((w) => w.replace(/\b\w/g, (c) => c.toUpperCase()));
  } else if (/^with the chosen name$/.test(q)) r.filter.nameIsChosen = 'cardName';
  else if ((m = q.match(/^with (flanking|banding|exalted|bushido|soulbond|exploit|melee|mentor|myriad|extort|afterlife|training|backup|decayed|toxic|annihilator|rampage)$/))) r.filter.keywords = [m[1].replace(/^\w/, (c) => c.toUpperCase())];
