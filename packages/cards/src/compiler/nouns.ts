...
function applyQualifier(q: string, r: ParsedNoun) {
  ...
  else if ((m = q.match(/^whose controller was dealt (combat )?damage by (?:~|it) this turn$/))) r.filter.controllerDamagedBySource = m[1] ? 'combat' : 'any'; else if ((m = q.match(/^with a (\+1\/\+1|-1\/-1|loyalty|charge) counter on (?:it|them)$/))) r.filter.hasCounter = m[1];
